...
async function loadCustomerDetails() {
    try {
        const response = await fetch(`/api/customer-details?customer_id=${customerId}`);
        const data = await response.json();
            ...
            document.getElementById('customer-joined').textContent = new Date(customer.creation_date).toLocaleDateString();
        } else {
...
async function loadCustomerTests() {
    try {
        const response = await fetch(`/api/customer-tests?customer_id=${customerId}`);
        const data = await response.json();
        ...
        if (data.success) {
            ...
            if (tests.length === 0) {
                ...
                tbody.innerHTML = tests.map(test => `
                    <tr>
                        <td>${test.id}</td>
                        <td><a href="#" class="text-decoration-none fw-bold clickable-link" onclick="filterTargetsByTest(${test.id}, '${escapeHtml(test.test_name || 'Untitled')}')" title="Click to view targets for this test">${escapeHtml(test.test_name || 'Untitled')}</a></td>
                        <td>${escapeHtml(test.test_description || 'No description')}</td>
...
async function loadCustomerTargets() {
    try {
        const response = await fetch(`/api/customer-targets?customer_id=${customerId}`);
        const data = await response.json();
...
async function loadCustomerVulnerabilities() {
    try {
        const response = await fetch(`/api/customer-vulnerabilities?customer_id=${customerId}`);
        const data = await response.json();
...
function displayVulnerabilities(vulnerabilities) {
    ...
    if (vulnerabilities.length === 0) {
        container.innerHTML = '<div class="text-center p-4 text-muted">No vulnerabilities found</div>';
    } else {
        container.innerHTML = vulnerabilities.map(vuln => createVulnerabilityCard(vuln)).join('');
    }
}
...
function createVulnerabilityCard(vuln) {
    ...
    return `
        <div class="vulnerability-card">
            <div class="vulnerability-header" onclick="toggleVulnerabilityDetails(${vuln.id})">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <h6 class="mb-1 text-primary">
                            <strong>ID: ${vuln.id}</strong> - ${escapeHtml(vuln.affected_entity || 'Unknown Entity')}
                        </h6>
                        <small class="text-muted">
                            Test: <strong>${escapeHtml(vuln.test_name || 'Unknown')}</strong>
                            |
                            Target: <strong>${escapeHtml(vuln.target_name || 'Unknown')}</strong>  
                        </small>
                    </div>
                    <div class="col-md-4 text-end">
                        <span class="cvss-score cvss-${cvssClass}">${vuln.cvss_score || 'N/A'}</span>
                        <span class="status-badge status-${statusClass} ms-2">${vuln.solved ? 'Solved' : 'Open'}</span>
                    </div>
                </div>
                ...
                <div class="vulnerability-details" id="vuln-details-${vuln.id}">
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${escapeHtml(vuln.identifier || 'Not specified')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Classification:</span>
                            <span class="detail-value">${escapeHtml(vuln.classification || 'Not specified')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Location:</span>
                            <span class="detail-value">${escapeHtml(vuln.location || 'Not specified')}</span>
                        </div>
                    </div>
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${escapeHtml(vuln.risk_statement || 'Not provided')}</span>
                        </div>
                        <div class="detail-row">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value"><code>${escapeHtml(vuln.cvssv3_code || 'Not provided')}</code></span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Likelihood:</span>
                            <span class="detail-value">${escapeHtml(vuln.likelihood || 'Not assessed')}</span>
                        </div>
                    </div>
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${escapeHtml(vuln.affected_component || 'Not specified')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Description:</span>
                            <span class="detail-value">${escapeHtml(vuln.vulnerabilities_description || 'No description provided')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Reproduction Steps:</span>
                            <span class="detail-value">
                                <pre class="bg-light p-2 rounded" style="white-space: pre-wrap; font-size: 0.9rem;">${escapeHtml(vuln.reproduction_steps || 'No reproduction steps provided')}</pre>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${escapeHtml(vuln.impact || 'Not assessed')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Residual Risk:</span>
                            <span class="detail-value">${escapeHtml(vuln.residual_risk || 'Not assessed')}</span>
                        </div>
                    </div>
                    ...
                    <div class="detail-section">
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">${escapeHtml(vuln.remediation_difficulty || 'Not assessed')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Recommendations:</span>
                            <span class="detail-value">${escapeHtml(vuln.recommendations || 'No recommendations provided')}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Recommended Reading:</span>
                            <span class="detail-value">${escapeHtml(vuln.recommended_reading || 'None provided')}</span>
                        </div>
                    </div>

                    <!-- Status -->
                    <div class="detail-section">
                        <h6><i class="bi bi-chat-left-text"></i>Status</h6>
                        <div class="detail-row">
                            <span class="detail-label">Status:</span>
                            <span class="detail-value">
                                <span class="status-badge status-${statusClass}">${vuln.solved ? 'Solved' : 'Open'}</span>
                            </span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Created:</span>
                            <span class="detail-value">${vuln.created_at ? new Date(vuln.created_at).toLocaleString() : 'Unknown'}</span>
                        </div>
                    </div>
                </div>
            </div>
...
function toggleVulnerabilityDetails(vulnId) {
    ...
    const indicator = details.closest('.vulnerability-card').querySelector('.expand-indicator');
...
function filterTargetsByTest(testId, testName) {
    ...
    const filteredTargets = allTargets.filter(target => target.test_id == testId);
...
function filterVulnerabilitiesByTarget(targetId, targetName) {
    ...
    const filteredVulnerabilities = allVulnerabilities.filter(vuln => vuln.target_id == targetId);
    
    // Switch to vulnerabilities tab using Bootstrap API
    const vulnTab = new bootstrap.Tab(document.getElementById('vulnerabilities-tab'));
    vulnTab.show();
        // Update header to show filter status
    setTimeout(() => {
